function TorqueDrag(DP_type)

clc;
%% Initial Inputs
[A,I,E,Ys,Wbp,f1,ro,ri,T_const,MW]=Data_Sheet(DP_type);

mu=f1;                           %friction factor
WBHA=27750;                      %Weight of BHA (lbf)
TBHA=0;                          %Torque at the bit/BHA
rp=ro/(12);                      %drillpipe outer radius (ft)
Rb=10000;                          %Build-up radius (ft)
Rd=0;                          %Drop-off radius (ft)
V=[1200,1200;1,2]; %Depth (ft) at which each segment(vertical/build-up/slant/drop-off) ends
